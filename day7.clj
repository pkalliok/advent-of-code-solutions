...
(let [[p w ch] (prop-maps (parse-input raw-input))]
  (def programs p) (def weight w) (def children ch))
...
(let [parent (smallest-unbalanced programs)
      ch (children parent)
      [wrong-weight right-weight] (->> ch (map total-weight) frequencies
                                    (map reverse) (map vec) sort (map second))
      wrong-ch (first (filter #(= wrong-weight (total-weight %)) ch))
      fixed-weight (+ (weight wrong-ch) right-weight (- wrong-weight))]
  (println fixed-weight))
